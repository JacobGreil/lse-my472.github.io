...
```{r}
authentication <- list(consumer_key = "Wdt3H4dd7Bey4DuzPGBeBdqZL",
                 consumer_secret = "uN5FIkEOCce1EL7v6f26NoxsILQBeWlu7ReoV6CBzQ0szv0EFI",
                 access_token = "269392564-fYP4DN7HcMAoP3mKWUHDLv3VIz7oJnIUtkhVfvEC",
                 access_token_secret = "Oho7oWV89eoa58DiO6Q5QHpB8yJPO2WeWFSKnc4jaQDRg")
```
...
```{r}
keywords_tweets <- stream_tweets("COP OR Biden", timeout = 5)
...
```{r}
stream_tweets(q = lookup_coords("uk"), timeout = 30, file_name='TweetsUK.json')
#stream_tweets(q = lookup_coords("usa"), timeout = 30, file_name='__________________')

geo_tweets <- tryCatch({parse_stream('TweetsUK.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'TweetsUK.json'));
                           return(recover_stream('TweetsUK.json'))})
```
